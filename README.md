...
An Obsidian plugin that shows an **anchor icon** in the editor gutter next to headings that are targeted by internal links (`[[Note#Heading]]` or `[[#Heading]]`). Click the icon to open a menu of all notes that link to that heading, then pick a note to jump straight to the link in the source file.
...
- **Quick navigation**: Click an anchor to open a context menu listing every source note (file name). Choosing an entry opens that note in Obsidian.
...
4. **Click** the anchor to see all notes that link to this heading; **click** a note name to open it.
...
**Context menu**  
Click an anchor to list every note that links to that heading, then open the one you need.
...
- The menu lists **source files**; opening a file follows Obsidian’s default behavior (cursor position may not jump to the exact link line in all cases).
