...
Плагин для Obsidian, который показывает **иконку якоря** в канале редактора (gutter) слева от заголовков, на которые ссылаются внутренние ссылки (`[[Заметка#Заголовок]]` или `[[#Заголовок]]`). Нажмите на иконку, чтобы открыть меню со всеми заметками, ссылающимися на этот заголовок, и перейти прямо к точной строке со ссылкой.
...
- **Быстрая навигация**: клик по якорю открывает контекстное меню со списком исходных заметок. Выбор пункта переводит к точной строке ссылки, а для заметок с несколькими вхождениями открывается второй список вида `строка: цитата`.
- **Переименование заголовка**: в контекстном меню есть действие **Переименовать этот заголовок…** (вызывает штатную команду Obsidian), которое переименовывает заголовок и обновляет все ссылки на него.
...
4. **Нажмите** на якорь, чтобы увидеть все заметки со ссылкой на этот заголовок; **нажмите** имя заметки, чтобы перейти к строке со ссылкой, или выберите нужное вхождение, если ссылок несколько.
...
**Контекстное меню**  
Клик по якорю открывает список заметок со ссылками на этот заголовок; затем можно перейти к точной строке нужной ссылки.
...
> **Примечание:** Этот способ станет доступен после принятия плагина в каталог сообщества Obsidian ([PR](https://github.com/obsidianmd/obsidian-releases/pull/11688)). Пока используйте установку через BRAT или вручную (см. ниже).
...
- Для заметок с несколькими ссылками на один и тот же заголовок открывается второй список, чтобы выбрать точное вхождение.
